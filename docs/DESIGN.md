...
└── ui/
    ...
    └── loop.ts     # 入力ループ・シグナルハンドリング・ステータスログ生成
```
...
どちらのパーサーの出力に対しても、ペアリングは共通ロジックで行う。詳細な実装仕様は[SPEC.md](./SPEC.md)を参照。設計上のポイント:

- **入力をミューテートしない** — 常に新しい配列を返す。
- バッククォートとチルダを独立に処理（同一 `nextPairId` カウンタを共有）。
- アクション検証のシミュレーションをサポートするため、明示的な `kind: "open"` 設定を尊重する。
...
この分離により、`fence-editor input.md | grep "code"` のようなパイプラインで、UIはターミナルに表示され、ドキュメントデータのみがパイプを流れる。詳細なUI仕様は[SPEC.md](./SPEC.md)を参照。

### 入力ループ

rawモードは使用しない。`runtime.ts` の `readLine()` で1行読み取り。空行・空白・非数値の入力はエラーメッセージを出さず静かに再プロンプトする。

### Ctrl+C ハンドリング

ループ開始時に `Deno.addSignalListener("SIGINT", ...)` を登録し、即座に終了コード0でabortする。ファイル変更は一切保存されない。未処理の例外は発生しない。

### フッター行

Actionsリストの直下にヒント行を毎回の再描画で必ず表示する。詳細な仕様は[SPEC.md](./SPEC.md)を参照。

### Actions ヘッダー

簡潔な `Actions:` のみを表示。冗長な説明は含めない（フッター行がその役割を果たす）。

### Actions の生成

`generateValidActions(state: EditorState): Action[]` が毎回動的に生成する。アクション型は `"restructure"` と `"convert-tilde"` の2種類のみ。アクションは `pairFences()` シミュレーションによって検証され、ドキュメント全体で完全に有効なペアリングを生成するアクションのみが表示される。

1. ペアワイズクローズスワップ（主要）
2. シングルクローズチェンジ（未ペアトークン向けフォールバック）

クローズフェンス変更候補は、結果のトークンリストをシミュレートし、ペアリングアルゴリズムを再実行することで検証される。すべてのフェンスが完全にペアリングされる（ゼロオーファンなし）アクションのみが提示される。
...
```sh
deno run --allow-read jsr:@tksh/fence-editor input.md                    # stdoutのみ（[3]）
deno run --allow-read --allow-write jsr:@tksh/fence-editor input.md      # 保存/上書きあり（[1]/[2]）
```
