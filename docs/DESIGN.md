...
どちらのパーサーの出力に対しても、ペアリングは共通ロジックで行う。詳細な実装仕様はSPEC.mdを参照。設計上のポイント:
...
この分離により、`fence-editor input.md | grep "code"` のようなパイプラインで、UIはターミナルに表示され、ドキュメントデータのみがパイプを流れる。詳細なUI仕様はSPEC.mdを参照。
...
Actionsリストの直下にヒント行を毎回の再描画で必ず表示する。詳細な仕様はSPEC.mdを参照。
