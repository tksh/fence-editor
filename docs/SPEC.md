...
1. `Format: <FORMAT>` 行（左揃え。`CommonMark` または `Djot`）
2. `Legend:` ブロック
3. `Status:` テーブル
...
| `raw`           | string                   | 元の行(80字でtruncate)     |
| `backtickCount` | number                   | フェンス長                 |
| `symbol`        | `"backtick" \| "tilde"`  | 記号種別                   |
| `infostring`    | `string      \| null`    | 言語指定子(終了はnull)     |
| `kind`          | `"open"      \| "close"` | 開始/終了                  |
...
- **restructure**: クローズチェンジまたはスワップを実行。
  - `kind`, `pairId` を更新
  - `pairFences()` 再実行
  - `autoAdjustBackticks()`
  - `raw` 再生成
...
- **convert-tilde**: 全 `tilde` トークンを `backtick` に変換。
  - `raw` 再生成
  - `autoAdjustBackticks()` でネスト調整
  - `hasTilde: false`
