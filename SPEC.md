...
テーブルセル内のバッククォート文字はそのまま書き出される（HTMLエンティティエンコードなし）。これは安全である。コードフェンスは行頭にバックティックシーケンスが必要であり、それはテーブルセル内では発生しない。
...
- ログ書き出し時のバッククォート文字のHTMLエンティティエンコード（意図的に除外；プレーンテキスト可読性のため生のバッククォートを使用）
